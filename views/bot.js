...
async function handleAnalizCommand(chatId, text) {
  try {
    ...
    const tf = parts[2] || '1h';
    // If user typed only '/analiz' (optionally with bot username), show usage helper
    if (parts.length === 1 || (parts.length === 2 && parts[1].startsWith('@'))) {
      ...
      try {
        ...
        // Fallback to short hint
        await bot.sendMessage(chatId, '⚡ /analiz Kullanımı
...
', { parse_mode: 'Markdown' });
    ...
    if (!subsinfo.isSubscriber) {
      // Free tier: ensure row and rate limit 1/day
      await ensureFreeUser(chatId);
      const allowed = await canUseAnaliz(chatId);
      if (!allowed) {
        return bot.sendMessage(chatId, getMessage('freeAnalizLimitReached') || '🆓 Free üyelikte /analiz günde 1 kez kullanılabilir. Sınırsız kullanım için lütfen abone olun.');
      }
    }
